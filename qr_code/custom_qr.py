...
#TODO: make it more faster: by generating the image in a single loop

class CustomQR:

    def draw_qr(self, matrix, background = (255,255,255), block_style = {"size": 10, "type" : 0, "color":[(0,0,0)], "pattern": 0}, finder_style = {"color":(0,0,0), "type":0},alignment_style = None, text_style = None):
        ...
        img = np.full((real_size, real_size, 3), background, dtype=np.uint8)

        img = self._draw_blocks(img, matrix, block_style, background)
        self._draw_finder(img, matrix_size, finder_style, block_size, background)
        self._draw_alignment(img, matrix_size, alignment_style, block_size, background)

        return img

    def _draw_blocks(self, img, matrix, block_style, background):
        if block_style["type"] == 0:
            self._draw_blocks_square(img, matrix, block_style)
        elif block_style["type"] == 1:
